			...
			success : function(e){
				...
				if(a.logicCode == '-1'){
					...
					if(totalCommodityInThisType == 0){
						if(confirm("该类型下目前包含话题数量为：【"+totalCommodityInThisType+"】可以放心删除，确定删除该类型吗？")==false)
							return;
						typeListComp.delType(typeId);
					}else{
						alert("该类型下目前包含话题数量为：【" + totalCommodityInThisType+"】，无法删除!" );
					}
				}
			}
		});
									...
									.each(
											function(i, obj) {
												b += '<tr><td>'
														...
														+ '</td>'
														+ '<td>'
														+ (typeof(obj.extra) == 'undefined' ? '': obj.extra)
														+ '</td>'
														+ '<td><a onclick="typeListComp.editClicked(\'' + obj.name + '\',' + obj.sequence + ',\'' + obj.id + '\');" href="#editModal" data-toggle="modal">编辑</a>' + 
														' / ' + 
